...
$.post('model/inword_entry.php',function(data){
					...
					$.post('model/cert_gen.php', {cert_id:cert_id},function(data){
						 ...
						 // $('.datasheet_popup_modal_body').load("g.htm");
             		    
             		    $('.aftersave').load("g.htm");
             		    $('.aftersave').find('#datasheet_table').html(data);
        			 // $('#datasheet_popup').modal('show');
	...
	$.post('model/inword_instrument.php',{current_id:current_id},function(data){
		...
		switch($current_val)
		{
			...
			case "Setting Standard" :
				...
				$('.instruadd_table').append('<table class="table table-bordered dynamic_table"><thead style="text-align:center;"><tr><th>Size</th><th>Make</th><th>ID No.</th></tr></thead><tbody><tr><td width="30%"><input type="text" id="instru_size_min" name="instru_size_min" class="form-control" style="display:inline; width:40%; ">-<input type="text" id="instru_size_max" name="instru_size_max" class="form-control form-inline" style="display:inline; width:40%;"></td><td><input type="text" id="instru_make" name="instru_make" class="form-control"></td><td><input type="text" id="instru_idno" name="instru_idno" class="form-control"></td></tr></tbody></table>');
			break;
